...
//	Used when recording a solution to get the time took
	public static void recordSolutionTime(int[] queenPos, Chromo X){
		if (Search.found_sol == false){
			...
			if (NQueensUtil.validateSolutionQuietly(Search.first_solution_board) == false){
				try{
					recordBadSolution(X);
				}
				catch(IOException e){
	...
	public static void recordBadSolution(Chromo X) throws IOException{
		// Open results.csv
		FileWriter fw = new FileWriter("results/badOutputs " + Parameters.numGenes + ".csv",true);
		BufferedWriter bw = new BufferedWriter(fw);
		// Generate output string
		String output = Parameters.numGenes + "," + String.valueOf(Search.first_sol_time_ns-Search.start_time_ns)+ "," + Search.curGen + "," + Parameters.popSize + "," + Parameters.selectType + "," + Parameters.scaleType + "," + Parameters.xoverType + "," + Parameters.xoverRate + "," + Parameters.mutationType + "," + Parameters.mutationRate + "," + Parameters.valueRepresentation + "," + Parameters.fitnessFunctionType + "," + X.rawFitness + "," + Arrays.toString(X.chromo) + ",";
		// Write it
