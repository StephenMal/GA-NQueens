...
def main():
    ...
    scaleList = [0]#,1] #2
    xoverTypeList_reg = [1,5]#,2,3,4,5,9] # 9
    xoverTypeList_key = [6]#,7,8]
    xoverRateList = [1]#[1, 5, 10, 20, 50] #5
    mutTypeList_reg = [1,2,3,4,5]
    mutTypeList_key = [6,7]#,8]
    mutRateList = [1]#, 5, 10, 20, 50] #5
    nQueensLength = 4 # 4 (how many we can do at once)
    nQueenStart = 8 # What queen to start at
    nextQueen = nQueenStart + nQueensLength # What the next queen will be
    maxQueen = 60
    nQueensList = list(range(nQueenStart, nQueenStart + nQueensLength))
    repList = [1]#,2] #2
    fitFxnList = [1] #,2,3,4,5,6] #6

    testsDone = 0
    totalTests = len(popList) * len(selectionList) * len(scaleList) * (len(xoverTypeList_reg)+len(xoverTypeList_key)) * len(xoverRateList) * (len(mutTypeList_reg) + len(mutTypeList_key)) * len(mutRateList) * len(fitFxnList)
                    ...
                    for xover_rate in xoverRateList:
                        for pop in popList:
                            ...
                            for xover_type in xoverTypeList_key:
                                for mutation_type in mutTypeList_key:
                                    rep_type = 2
                                    arg = {}
                                    arg['pop'] = pop
                                    arg['select'] = select
    ...
    while(nextQueen <= maxQueen):
        for index in range(0,nQueensLength):
            queen = nQueensList[index]
            # Check if queen is already completed, if so reset it and set to next queen#
            if (queenComplete[queen] is True):
                #If we have hit our max just skip
                if nextQueen == maxQueen:
                    continue
                else: #Otherwise make a new queen
                    queen = nextQueen
                    nQueensList[index] = nextQueen
                    nextQueen = nextQueen + 1
                    qProc[queen] = Process(target=runSearch, name = "Queen"+str(queen), args=(arguments[qArgIndex[queen]],queen))
                    qProc[queen].start()
                    qTime[queen] = time.time() + calcWaitTime(arguments[qArgIndex[queen]],queen)
                    genOutputFiles(queen)
